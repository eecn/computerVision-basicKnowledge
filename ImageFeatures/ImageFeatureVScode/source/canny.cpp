...
void feat::getCannyEdge(const Mat& imgSrc, Mat& imgDst, double lowThresh, double highThresh, double sigma)
{
    ...
    if (possibleWidth == 30)
    {
        possibleWidth = 1; // the user entered a reallly small sigma   当sigma很小时，size就是30
    }

    // get the 1D gaussian filter  构造一维高斯滤波器
    ...
    // get the derectional derivatives of gaussian kernel   得到高斯核的正则导数 二维的
    ...
    if (magMax > 0 )
    {
        imgMag = imgMag / magMax;    //归一化到0-1之间
    }

    // 计算指定百分比下的高阈值 与对应的低阈值  这个低阈值其实也可以通过百分比求得 但是其实意义一样
    ...
    for (int dir = 1; dir <= 4; dir++)
    {
        ...
        imgStrong= imgStrong| ((imgMag > highThresh) & imgWeak);   //一定是边缘
...
double feat::getCannyThresh(const Mat& inputArray, double percentage)
{
    ...
    double step = (maxValue - minValue) / nBins;  //化为 nBins个灰度值

	//统计灰度直方图  这里理论上不用+1 但是出现vector out of range  这里+1 并不影响计算结果
    vector<unsigned> histBin(nBins+1,0);
    ...
    for (int i = 0; i < nBins; i++)
    {
        ...
        if (cumSum > percentage * inputArray.rows * inputArray.cols)  // 累积到设定的百分比 之后输出对应的百分比作为阈值   这里返回的是阈值上限
        {
            thresh = (i + 1) / 64.0;
            break;
        }
